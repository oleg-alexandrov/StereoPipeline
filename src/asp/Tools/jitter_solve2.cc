...
struct Options : vw::cartography::GdalWriteOptions {
  std::string left_image, right_image, left_camera, right_camera, out_prefix,
    images_to_use, mode, bundle_adjust_prefix;
...
struct ReprojectionError {
  ReprojectionError(int sign, Vector2 const& observation,
                    Vector3 const& translation, 
                    Vector3 const& axis_angle, 
                    CamPtr cam, 
                    std::vector<int> const& block_sizes):
    m_sign(sign),
    m_observation(observation),
    m_translation(translation),
    m_axis_angle(axis_angle),
    m_cam(cam),
...
//        std::cout << "--block size is " << it << " " << m_block_sizes[it] << std::endl;
    ...
    AdjustedCameraModel * adj_cam = dynamic_cast<AdjustedCameraModel*>(m_cam.get());
    if (adj_cam == NULL) 
      vw::vw_throw( vw::ArgumentErr() << "Expecting an adjusted camera.\n\n");
    
    asp::DGCameraModel * dg_cam
      = dynamic_cast<asp::DGCameraModel*>(adj_cam->unadjusted_model().get());
    
    if (dg_cam == NULL) 
      vw::vw_throw( vw::ArgumentErr() << "Expecting a Digital Globe camera.\n\n");

    // Create the camera
    boost::shared_ptr<asp::LinescanModelFreq> freq_cam(new asp::LinescanModelFreq(dg_cam));
...
//     std::cout << std::endl;
    ...
    try {
      ...
      AdjustedCameraModel acam(freq_cam);
      acam.set_translation(m_translation);
      acam.set_axis_angle_rotation(m_axis_angle);
          
      // Use the camera model wrapper to handle all of the parameter blocks.
      Vector2 prediction = acam.point_to_pixel(xyz);
  ...
  static ceres::CostFunction* Create(int sign,
                                     Vector2 const& observation,
                                     Vector3 const& translation,
                                     Vector3 const& axis_angle,
                                     CamPtr cam,
                                     std::vector<int> const& block_sizes){
    ceres::DynamicNumericDiffCostFunction<ReprojectionError>* cost_function =
      ...
      (new ReprojectionError(sign, observation, translation, axis_angle, cam, block_sizes));
...
// observed 3D point and the current (floating) 3D point.
struct XYZError {
  XYZError(Vector3 const& obs, double weight):
    m_obs(obs), m_weight(weight) {}
...
void compute_residuals(Options const& opt,
  ...
  if (opt.xyz_weight > 0) {
    num_expected_residuals += num_points * NUM_POINT_PARAMS; // xyz residuals
  }
...
void handle_arguments(int argc, char *argv[], Options& opt) {
  ...
  general_options.add_options()
    ...
    ("xyz-weight",      po::value(&opt.xyz_weight)->default_value(-1),
     "The weight to use to constrain the triangulated points to the initial values.")
    ...
    ("float-ms-offset",   po::bool_switch(&opt.float_ms_offset)->default_value(false),
     "Float the MS offset.")
    ("freeze-num",       po::value(&opt.freeze_num)->default_value(0),
     "If to freeze the first camera (0) or second camera (1).") 
...
//   if ( opt.horiz_weight < 0.0 || opt.vert_weight < 0 || opt.xyz_weight < 0)
//     vw_throw( ArgumentErr() << "The vertical, horizontal, and overall weights "
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    std::cout << "-num frequencies is " << opt.num_frequencies << "\n";
    ...
    // We assume no pixel offset in the adjusted model
    Vector3 left_translation = left_adj_cam->translation();
    Vector3 left_axis_angle = left_adj_cam->rotation().axis_angle();
    Vector3 right_translation = right_adj_cam->translation();
    Vector3 right_axis_angle = right_adj_cam->rotation().axis_angle();
          
    // TODO(oalexan1): Need to load just one camera and the MS offset

    std::vector<Vector3> translations, axis_angles;
    translations.push_back(left_translation);
    translations.push_back(right_translation);
    axis_angles.push_back(left_axis_angle);
    axis_angles.push_back(right_axis_angle);
    
    asp::DGCameraModel * left_dg_cam = dynamic_cast<asp::DGCameraModel*>
      (left_adj_cam->unadjusted_model().get());
    asp::DGCameraModel * right_dg_cam = dynamic_cast<asp::DGCameraModel*>
      (right_adj_cam->unadjusted_model().get());

    if (left_dg_cam == NULL || right_dg_cam == NULL) 
    vw::vw_throw( vw::ArgumentErr() << "Expecting Digital Globe cameras.\n\n");

    left_dg_cam->m_ms_offset = 0;
    right_dg_cam->m_ms_offset = -opt.ms_offset;

    left_dg_cam->m_coeffsx = std::vector<double>();
    left_dg_cam->m_coeffsy = std::vector<double>();
    left_dg_cam->m_coeffsz = std::vector<double>();
    
    right_dg_cam->m_coeffsx = std::vector<double>();
    right_dg_cam->m_coeffsy = std::vector<double>();
    right_dg_cam->m_coeffsz = std::vector<double>();

    std::cout << "ms_offset = " << left_dg_cam->m_ms_offset << " for " << opt.left_image << "\n";
    std::cout << "ms_offset = " << right_dg_cam->m_ms_offset << " for " << opt.right_image<< "\n";

    std::cout << "--horiz weight = " << opt.horiz_weight << "\n";
    std::cout << "--vert weight = " << opt.vert_weight << "\n";
    std::cout << "--xyz weight = " << opt.xyz_weight << "\n";
    
    std::vector<double> coeffs(2 * opt.num_frequencies + 1, 0);
    
    std::vector< std::vector<double> > coeffsx, coeffsy, coeffsz;
    coeffsx.push_back(coeffs); coeffsx.push_back(coeffs);
    coeffsy.push_back(coeffs); coeffsy.push_back(coeffs);
    coeffsz.push_back(coeffs); coeffsz.push_back(coeffs);

    std::cout << "export ADJX='";
    for (int it = 0; it < int(coeffsx[0].size()) - 1; it++) std::cout << coeffsx[0][it] << " ";
    std::cout << coeffsx[0].back() << "'\n";
    
    std::cout << "export ADJY='";
    for (int it = 0; it < int(coeffsy[0].size()) - 1; it++) std::cout << coeffsy[0][it] << " ";
    std::cout << coeffsy[0].back() << "'\n";

    std::cout << "export ADJZ='";
    for (int it = 0; it < int(coeffsz[0].size()) - 1; it++) std::cout << coeffsz[0][it] << " ";
    std::cout << coeffsz[0].back() << "'\n";

    
    // 2 cameras and images
    std::vector<std::string> images;
    std::vector<CamPtr> cameras;
    std::vector<int> sign;
    std::map<std::pair<int, int>, std::string> match_files;
    
    images.push_back(opt.left_image);
    cameras.push_back(left_cam);
    sign.push_back(0);
    
    images.push_back(opt.right_image);
    ...
    std::string match_file = ip::match_filename(opt.out_prefix, opt.left_image, opt.right_image);
    match_files[std::pair<int, int>(0, 1)] = match_file;

    std::vector<ip::InterestPoint> left_ip, right_ip;
    ip::read_binary_match_file(match_file, left_ip, right_ip);
    std::cout << "--number of matches: " << left_ip.size() << std::endl;
    ...
    std::cout << "Number of points in the network " << num_points << "\n";
    ...
    std::cout << "--crn size is " << crn.size() << "\n";
    int num_cameras = crn.size();

    double ms_offset = opt.ms_offset;
    std::cout << "export MS_OFFSET=" << ms_offset << "\n";
    ...
    for (int icam = 0; icam < num_cameras; icam++) { // Camera loop
      ...
      for (crn_iter fiter = crn[icam].begin(); fiter != crn[icam].end(); fiter++){ // IP loop
        ...
        ceres::CostFunction* cost_function =
          ReprojectionError::Create(sign[icam], observation,
                                    translations[icam], axis_angles[icam],
                                    cameras[icam], block_sizes);

        ceres::LossFunction* loss_function = get_jitter_loss_function();
        
        problem.AddResidualBlock(cost_function, loss_function,
                                 &coeffsx[icam][0], &coeffsy[icam][0], &coeffsz[icam][0],
                                 point, &ms_offset);
    ...
    if (opt.xyz_weight > 0) {
      ...
      for (int ipt = 0; ipt < num_points; ipt++){
        ...
        ceres::CostFunction* xyz_cost_function
          = XYZError::Create(observation, opt.xyz_weight);
        ceres::LossFunction* xyz_loss_function = get_jitter_loss_function();
        problem.AddResidualBlock(xyz_cost_function, xyz_loss_function, point);
    ...
    if (opt.mode == "x") {
      if (!opt.float_free_term) {
        // Freeze the free term, coeffsx[fn][0].
        problem.SetParameterization(&coeffsx[fn][0],
                                    new ceres::SubsetParameterization(coeffs.size(), {0}));
      }
      // Fix all y and z coeffs
      problem.SetParameterBlockConstant(&coeffsy[fn][0]);
      problem.SetParameterBlockConstant(&coeffsz[fn][0]);
    } else if (opt.mode == "y") {
      if (!opt.float_free_term) {
        // Freeze the free term, coeffsy[fn][0].
        problem.SetParameterization(&coeffsy[fn][0],
                                    new ceres::SubsetParameterization(coeffs.size(), {0}));
      }
      // Fix all x and z coeffs
      problem.SetParameterBlockConstant(&coeffsx[fn][0]);
      problem.SetParameterBlockConstant(&coeffsz[fn][0]);
      
    } else if (opt.mode == "z") {
      if (!opt.float_free_term) {
        // Freeze the free term, coeffsz[fn][0].
        problem.SetParameterization(&coeffsz[fn][0],
                                    new ceres::SubsetParameterization(coeffs.size(), {0}));
      }
    ...
    // Freeze second camera completely
    problem.SetParameterBlockConstant(&coeffsx[fy][0]);
    problem.SetParameterBlockConstant(&coeffsy[fy][0]);
    problem.SetParameterBlockConstant(&coeffsz[fy][0]);
    ...
    for (int icam  = 0; icam < num_cameras; icam++) {
      ...
      std::cout << "export ADJX='";
      for (int it = 0; it < int(coeffs.size()) - 1; it++) std::cout << coeffsx[icam][it] << " ";
      std::cout << coeffsx[icam].back() << "'\n";
    
      std::cout << "export ADJY='";
      for (int it = 0; it < int(coeffs.size()) - 1; it++) std::cout << coeffsy[icam][it] << " ";
      std::cout << coeffsy[icam].back() << "'\n";

      std::cout << "export ADJZ='";
      for (int it = 0; it < int(coeffs.size()) - 1; it++) std::cout << coeffsz[icam][it] << " ";
      std::cout << coeffsz[icam].back() << "'\n";

      std::cout << "export MS_OFFSET=" << ms_offset << "\n";

      std::ostringstream iss;
      iss << "_" << icam;
      std::string suff = iss.str();
        
      // Save coeffs to disk
      std::string jx = opt.out_prefix + "-adjx" + suff + ".txt";
      std::cout << "Writing: " << jx << "\n";
      std::ofstream ox(jx.c_str());
      ox.precision(18);
      for (int it = 0; it < int(coeffs.size()) - 1; it++)
        ox << coeffsx[icam][it] << " ";
      ox << coeffsx[icam].back() << "\n";

      std::string jy = opt.out_prefix + "-adjy" + suff + ".txt";
      std::cout << "Writing: " << jy << "\n";
      std::ofstream oy(jy.c_str());
      oy.precision(18);
      for (int it = 0; it < int(coeffs.size()) - 1; it++)
        oy << coeffsy[icam][it] << " ";
      oy << coeffsy[icam].back() << "\n";

      std::string jz = opt.out_prefix + "-adjz" + suff + ".txt";
      std::cout << "Writing: " << jz << "\n";
      std::ofstream oz(jz.c_str());
      oz.precision(18);
      for (int it = 0; it < int(coeffs.size()) - 1; it++)
        oz << coeffsz[icam][it] << " ";
      oz << coeffsz[icam].back() << "\n";
    }
    
    sw.stop();
    vw::vw_out() << "Jitter solve elapsed time: " << sw.elapsed_seconds() << "\n";
