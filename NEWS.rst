...
sparse_disp (:numref:`sparse_disp`):
  * Added the ability to produce subpixel-level match files (parabola subpixel)
    ...
    (:numref:`dsub_match`).
  * Added four per-match quality metrics: ``parabola_curvature`` and
    ``cramer_rao`` (:numref:`sparse_disp_uncertainty`), and ``peak_ratio`` and
    ``snr`` (:numref:`sparse_disp_quality_metrics`).
...
parallel_stereo (:numref:`parallel_stereo`):
  ...
  * The outlier removal for the low-resolution disparity ``D_sub.tif`` is now
    ...
    :numref:`filter_options` for details.
